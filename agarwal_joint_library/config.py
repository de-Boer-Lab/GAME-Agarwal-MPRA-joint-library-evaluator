...
evaluator_base = "agarwal_2025_joint_lib" 
...
if os.path.exists("/.singularity.d"):
    ...
    EVALUATOR_DATA_DIR = os.path.join(SCRIPT_DIR, "evaluator_data_small", "2023-03-03628-s5") # Subset of full data for local testing
    EVALUATOR_NAME = f"{evaluator_base}_dev"

EVALUATOR_INPUT_PATH = os.path.join(EVALUATOR_DATA_DIR, input_file)
PLASMID_BACKBONE_INPUT_PATH = os.path.join(EVALUATOR_DATA_DIR, "..", plasmid_backbone) # NOTE: Added plasmid backbone input path
output_filename_base = f'{EVALUATOR_NAME}_predictions' # NOTE: Removed input file from output filename to avoid overly long names
